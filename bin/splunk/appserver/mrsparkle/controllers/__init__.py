...
class BaseController(object):
    ...
    def redirect_to_url(self, *a, **kw):
        ...
        # not all URLs are safe for a server-side redirect in IE 6, fall back to a client-side redirect in that case
        if util.is_ie_6() and not util.redirect_url_is_ie_6_safe(redirect_url):
            return self.client_side_redirect(redirect_url)

        raise cherrypy.HTTPRedirect(redirect_url)

    def client_side_redirect(self, redirect_to):
        """
        Performs a redirect client-side by sending back a <script> tag that will do it in javascript.
        Can be used to normalize different browser behaviors with redirects and fragment ids.
        The contract is:
            - If the redirect URL has a fragment id, it will be respected and override any fragment id that is present
                in the client.
            - If the redirect URL has not fragment id, the fragment id that is present in the client (if any) will be
                appended when the redirect is made.
        """

        rendered_template = self.render_template('client_side_redirect.html', { 'redirect_to': redirect_to })
        # make sure we instruct the browser to not cache this document
        return util.set_cache_level('never', rendered_template)
