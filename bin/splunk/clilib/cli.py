...
def list_commands(args, fromCLI = True):
  ...
  for oneCmd in mainCommands:
    ...
    if cList.hasSubCmd(oneCmd):
      subCommands = cList.listSubCmds(oneCmd)
      subCommands.sort()
      logger.info("\t%s" % str.join(", ", subCommands))
    else:
...
def login(args, fromCLI = True):
  ...
  print_login_banner()
  username  = raw_input("Splunk username: ").strip()
...
def didYouMean(command):
    ...
    commands = ["search", "login", "logout", "start", "stop", "restart", "status", "add", "edit", "list", "import", "export", "clean", "summary", "enable", "disable", "display", "train", "test", "validate", "anonymize", "find", "help", "diag", "dispatch", "getconf", "install", "migrate", "package", "recover", "refresh", "reload", "remove", "set", "show", "spool", "upgrade"]
...
class CommandList:
  ...
  def hasCmd(self, cmdName):
    return cmdName in self.cmdList.keys()
  ...
  def getCmdEAICLI(self, cmdName, subCmd = None):
    #if not cmdName in self.cmdList.keys():
    ...
    if not isinstance(self.cmdList[cmdName], dict) or not subCmd:
      ...
      if not subCmd in self.cmdList[cmdName].keys():
  ...
  def getCmd(self, cmdName, subCmd = None):
    ...
    if not isinstance(self.cmdList[cmdName], dict) or not subCmd:
      ...
      return self.cmdList[cmdName][subCmd]
  def listCmds(self):
    ret = self.cmdList.keys()
    ret.sort()
    return ret
  def listSubCmds(self, name):
    subCmds = []
    if self.hasSubCmd(name):
      subCmds = self.cmdList[name].keys()
    return subCmds
...
cList.addCmd(SplunkCmd("edit",   "search-server",    None))
...
#cList.addCmd(SplunkCmd("_internal", "import-flat-files", index.importAllFlatFiles, authReq = False))
cList.addCmd(SplunkCmd("_py_internal", "import-flat-files", index.importAllFlatFiles, authReq = False))
...
def parseAndRun(argsList):
  ...
  if not cList.hasCmd(command):
    ...
    hasSubCmd = True
    subCmd    = None
    username  = password = authInfo = defArg = retVal = None
    ...
    if not noAuthReq:
        #EAI
        #if isProVersion:
        #if we require auth we also require splunk to be running
        if not comm.isProVersion():
          username = "admin"
    ...
    while not authWorked:
      ...
      if not (command in ['login'] or subCmd in ['check-xml-files', 'pre-flight-checks', 'first-time-run', 'import-flat-files']):
           try:
              ...
              if not argList.has_key('owner'): argList['owner'] = auth.getCurrentUser()['name'] 

              retVal = makeRestCall(cmd=command, obj=subCmd, restArgList=objUnicode(argList), sessionKey=authInfo, dotSplunk=dotSplunk) 
           ...
           ########
              ...
              raise e
           except (CliArgError,InvalidStatusCodeError),  e:
              raise e
           except NoEndpointError, e:
              ...
              if command in run_local or '%s:%s' % (command, subCmd) in run_local or '*:%s' % (subCmd) in run_local:
                 ...
                 if argList.has_key('authstr'):
                    argList.pop('authstr')
                 logger.error('The cmd "%s %s %s" is currently not supported in the CLI' % (command, subCmd, ' '.join(map(lambda x: '-%s %s' % (x[0], x[1]), argList.items()))  ))
                 raise e
      else:
           try:
             retVal = cList.getCmd(command, subCmd).call(argList, fromCLI = True)
    ...
    if isinstance(retVal, dict):
      if "restartRequired" in retVal.keys():
...
def getAuthFilePath():
  try:
      hostname_ep = splunk.entity.getEntity('/server/info', 'server-info').properties['serverName']
      hostname = hostname_ep
  except:
      #maybe splunk is down
      hostname = ''
...
def main(argsList):
  ...
  if len(argsList) < 2:
    ...
    except CliArgError, e:
       logger.error('\nAn error occurred:\n' + '\n'.join(e.args))
       return ERR_ARG

    except InvalidStatusCodeError, e:
       logger.error('\nAn error occurred:\n' + '\n'.join(e.args))
    ...
    #SPL-24352
    except splunk.SplunkdException, e:
       logger.debug(str(e))
       logger.error('\n'.join(e.args))
       return ERR_SPLUNKD 
...
if __name__ == "__main__":
  ...
  try:
    ...
    logger.getLogger().addHandler(comm.newLogHandler(sys.stdout, comm.debugMode, normal = True))
    logger.getLogger().addHandler(comm.newLogHandler(sys.stderr, comm.debugMode, normal = False))
