...
def entityParams(**kw):
    ...
    for key in ENTITY_PARAMS:
        if kw.has_key(key): resp[key] = kw[key]
    return resp
...
def unquoteEntity(entity_name):
    ...
    '''
    return urllib.unquote(entity_name).replace('%2F', '/')
...
def getEntity(entityPath, entityName, uri=None, namespace=None, owner=None, sessionKey=None, hostPath=None, **kwargs):
    ...
    try:
        ...
        contents = unicode(atomEntry).encode('UTF-8')
...
class Entity(object):
    ...
    def __str__(self):
        if self.value != None:
            ...
            return unicode(self.properties).encode('UTF-8')
        else:
            return ''
    ...
    def _parseContents(self, contents):
        ...
        # TODO: should be handle list objects here too?
            ...
            self.value = unicode(contents).encode('UTF-8')
