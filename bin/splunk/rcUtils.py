...
def layeredFind(endpoint, cmd, obj, target, parm=''):
   ...
   try:
      ...
      if remote_cmds[endpoint].has_key('%s:%s' % (cmd, obj)):
         try:
...
def handleHelp(endpoint, restArgList):
   ...
   if not restArgList.has_key('cmdname'): #in case they type ./splunk help
   ...
   if not help_text:
      ...
      try:
         l = [remote_cmds[k]['_common']['help'][restArgList['cmdname']] for k in remote_cmds.keys() if k != '_common' if remote_cmds[k]['_common'].has_key('help') if remote_cmds[k]['_common']['help'].has_key(restArgList['cmdname'])]
      except:
...
def handleSyncSearch(search, sessionKey, namespace, owner, argList):
   ...
   try: 
      ...
      #SPL-23022
      if not argList.has_key('timeout'):
         searchjob.cancel()
   except KeyboardInterrupt, e:
      try:
         searchjob.cancel()
...
# -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
def makeRestCall(cmd=None, restArgList=None, obj=None, sessionKey=None, entityName=None, search=None, count=None, offset=None, sort_key=None, sort_dir=None, dotSplunk=None):
   """
   ...
   #hack for removing all async jobs, to be removed when an EAI endpoint is provided to do this
   elif cmd == 'remove' and obj == 'jobs' and restArgList.has_key('jobid') and restArgList['jobid'] == 'all':
     return handleRemoveJobsAll(sessionKey, namespace, owner)
      ...
      if method == 'POST':
         ...
         if cmd == 'list' and obj in ['monitor', 'tail'] and eaiArgsList.has_key('show-hidden'):
