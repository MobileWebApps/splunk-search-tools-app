...
class Beta(Gauleg18):
    ...
    def betai(cls, a, b, x):
        ...
        if x < (a+1.0)/(a+b+2.0): return bt*cls.betacf(a,b,x)/a
        else: return 1.0 - bt*cls.betacf(b,a,1.0-x)/b
...
class Fdist(Beta):
    ...
    def cdf(self, f):
        if f < 0.: raise ValueError
        nu1, nu2 = self.nu1, self.nu2
        return self.betai(0.5*nu1,0.5*nu2,nu1*f/(nu2+nu1*f))
...
class Normaldist(Erf):
    ...
    def p(self, x):
        mu = self.mu
        sig = self.sig
        return (0.398942280401432678/sig)*exp(-0.5*((x-mu)/sig)**2)

    def cdf(self, x):
        mu = self.mu
        sig = self.sig
        return 0.5*self.erfc(-0.707106781186547524*(x-mu)/sig)
