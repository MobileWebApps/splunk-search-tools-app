...
import unittest, urllib, httplib2, cgi
import lxml.etree
...
class SimpleRestTester(object):
    ...
    def run(self, *filelist):
        ...
        self.log(0, '')
        self.log(0, '-' * 76)
        self.log(0, 'suite time: %.3f sec; %s files(s): %s pass; %s fail' % (time.time() - t1, fileCount, passCount, failCount))
    ...
    def assertEqual(self, a, b, msg=None):
        if a != b:
            if type(a) == type('') and len(a) > TRUNCATE_LENGTH: a = a[:TRUNCATE_LENGTH] + '...'
            if type(b) == type('') and len(b) > TRUNCATE_LENGTH: b = b[:TRUNCATE_LENGTH] + '...'
        ...
        try:
            ...
            while i < len(string):
                ...
                if match:
                    ...
                    if varName in self.variables or varName == "SPLUNK_HOME":
                        ...
                        # Replacement might be shorter than original string.
                        # Set current position based on new length.
                        # Let i point to one past the replacement string,
                        # keeping in mind that replacement string may contain
                        # escape chars, hence len(replacement) is not reliable
                        i = match.start(1) + varNameLen + (len(string) - stringOrigLen) + 1
                    else:
                        i = match.end(1)
                else:
                    break
    ...
    def executeTest(self, filepath):
      try:   
        ...
        # look for HTTP digest auth into
        ...
        if authInfo is not None:
            ...
            self.log(2, 'Using credentials: %s/%s' % (username, password))
    ...
    def execute_test_case(self, test):
        ...
        if not payload:
            for arg in requestParams.findall('form/arg'):
                ...
                if arg.get('save'):
                    self.saveVariable(arg.get('save'), self.replaceVariables(arg.text))
            payload = urllib.urlencode(formList)
            if len(formList) > 0:
        ...
        try:
            ...
            if querystring:
                uri += '?' + urllib.urlencode(querystring)
